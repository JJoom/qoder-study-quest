...
> **总体提案与后续模块提案的关系（重要）**：这个提案是 QShop 的「总蓝图」，**只审查、不要归档**。OpenSpec 的归档会把 delta 规格合并进主规格，若提前归档，等于把尚未实现的功能写成「系统已有行为」，污染真相源。第 07 讲起，每个模块的实现由各自的模块级提案驱动（完成一个归档一个，主规格随之生长）；这个总体变更留着当课程路线图即可。
